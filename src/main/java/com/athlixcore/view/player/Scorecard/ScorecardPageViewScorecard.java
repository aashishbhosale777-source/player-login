...
public class ScorecardPageViewScorecard {
    ...
    public Node getView() {
        ...
        VBox matchSummaryCard = buildMatchSummary();

        // --- 3. THIS OVER TIMELINE ---
        VBox overTimelineCard = buildOverTimeline();

        // --- 4. SCORECARD CONTENT CARD ---
        VBox scorecardCard = new VBox();
        scorecardCard.setStyle("-fx-background-color: #ffffff; -fx-background-radius: 8; -fx-border-color: #e2e8f0; -fx-border-radius: 8; -fx-effect: dropshadow(three-pass-box, rgba(0,0,0,0.05), 10, 0, 0, 4);");
        ...
        mainContainer.getChildren().addAll(header, matchSummaryCard, overTimelineCard, scorecardCard);
    ...
    private VBox buildMatchSummary() {
        VBox summaryCard = new VBox(15);
        summaryCard.setPadding(new Insets(20, 30, 20, 30));
        summaryCard.setStyle("-fx-background-color: #ffffff; -fx-background-radius: 8; -fx-border-color: #e2e8f0; -fx-border-radius: 8; -fx-effect: dropshadow(three-pass-box, rgba(0,0,0,0.03), 8, 0, 0, 2);");
        ...
        Label t1Name = new Label(team1);
        t1Name.setStyle("-fx-font-weight: bold; -fx-font-size: 16px; -fx-text-fill: #475569;");
        Label t1Score = new Label("158/2 (15.0 Ov)");
        t1Score.setStyle("-fx-font-weight: bold; -fx-font-size: 26px; -fx-text-fill: #0f172a;");
        team1Box.getChildren().addAll(t1Name, t1Score);

        // VS Label
        Label vsLbl = new Label("VS");
        vsLbl.setStyle("-fx-font-weight: bold; -fx-font-size: 20px; -fx-text-fill: #cbd5e1;");

        // Team 2 Block
        VBox team2Box = new VBox(5);
        team2Box.setAlignment(Pos.CENTER);
        Label t2Name = new Label(team2);
        t2Name.setStyle("-fx-font-weight: bold; -fx-font-size: 16px; -fx-text-fill: #475569;");
        Label t2Score = new Label("160/2 (15.0 Ov)");
        t2Score.setStyle("-fx-font-weight: bold; -fx-font-size: 26px; -fx-text-fill: #0f172a;");
        team2Box.getChildren().addAll(t2Name, t2Score);

        scoreRow.getChildren().addAll(team1Box, vsLbl, team2Box);

        // Match Status/Result Footer
        Label statusLbl = new Label(team2 + " won the match");
        statusLbl.setStyle("-fx-font-size: 14px; -fx-font-weight: bold; -fx-text-fill: #10b981; -fx-background-color: #ecfdf5; -fx-padding: 6 16; -fx-background-radius: 20;");
        HBox statusBox = new HBox(statusLbl);
        statusBox.setAlignment(Pos.CENTER);

        summaryCard.getChildren().addAll(scoreRow, statusBox);
        return summaryCard;
    }

    private VBox buildOverTimeline() {
        VBox timelineCard = new VBox(15);
        timelineCard.setPadding(new Insets(20));
        timelineCard.setStyle("-fx-background-color: #ffffff; -fx-background-radius: 8; -fx-border-color: #e2e8f0; -fx-border-radius: 8; -fx-effect: dropshadow(three-pass-box, rgba(0,0,0,0.03), 8, 0, 0, 2);");

        Label title = new Label("This Over Timeline");
        title.setStyle("-fx-font-size: 14px; -fx-text-fill: #334155;");

        HBox ballsBox = new HBox(12);
        ballsBox.setAlignment(Pos.CENTER_LEFT);
        
        ballsBox.getChildren().addAll(
            createBallBadge("4", "boundary"),
            createBallBadge("4", "boundary"),
            createBallBadge("Nb", "extra"),
            createBallBadge("W", "wicket"),
            createBallBadge("Wd", "extra"),
            createBallBadge("6", "boundary")
        );

        timelineCard.getChildren().addAll(title, ballsBox);
        return timelineCard;
    }

    private StackPane createBallBadge(String text, String type) {
        StackPane badge = new StackPane();
        badge.setPrefSize(45, 45);
        badge.setMinSize(45, 45);
        ...
        switch(type) {
            case "boundary":
                bgColor = "#ecfdf5"; borderColor = "#a7f3d0"; textColor = "#047857";
                break;
            case "wicket":
                bgColor = "#fef2f2"; borderColor = "#fecaca"; textColor = "#dc2626";
                break;
            case "extra":
                bgColor = "#fffbeb"; borderColor = "#fde68a"; textColor = "#b45309";
                break;
            default:
                bgColor = "#f8fafc"; borderColor = "#e2e8f0"; textColor = "#475569";
                break;
        }

        badge.setStyle("-fx-background-color: " + bgColor + "; -fx-border-color: " + borderColor + "; -fx-border-width: 1px; -fx-border-radius: 22.5; -fx-background-radius: 22.5;");
