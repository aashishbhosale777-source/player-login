...
public class CurrentTournamentSquad {
    ...
    public Node getView() {
        ...
        // Add Mock Players with Updated Images for P1 and P2
        playersGrid.getChildren().addAll(
            createPlayerCard("/assests/images/CurrentTournamentSquad2.png", "Player One", "👑 Captain / Batter"),
            createPlayerCard("/assests/images/CurrentTournamentSquad1.png", "Player Two", "🏏 Top Order Batter"),
            createPlayerCard("https://dummyimage.com/100x100/cbd5e1/0f172a.png&text=P3", "Player Three", "🧤 Wicket Keeper"),
            createPlayerCard("https://dummyimage.com/100x100/cbd5e1/0f172a.png&text=P4", "Player Four", "⚔️ All-Rounder"),
            createPlayerCard("https://dummyimage.com/100x100/cbd5e1/0f172a.png&text=P5", "Player Five", "⚾ Fast Bowler"),
            createPlayerCard("https://dummyimage.com/100x100/cbd5e1/0f172a.png&text=P6", "Player Six", "🌀 Spin Bowler")
    ...
    private VBox createPlayerCard(String imagePath, String playerName, String role) {
        VBox card = new VBox(10);
        card.setAlignment(Pos.CENTER);
        card.setPadding(new Insets(20));
        card.setPrefWidth(180);
        ...
        if (imagePath != null && !imagePath.isEmpty()) {
            try {
                if (imagePath.startsWith("http")) {
                    profileImg.setImage(new Image(imagePath, true));
                } else {
                    String cleanPath = imagePath.startsWith("/") ? imagePath.substring(1) : imagePath;
                    File file = new File("src/main/resources/" + cleanPath);
                    if (file.exists()) {
                        profileImg.setImage(new Image(file.toURI().toString(), true));
                    } else {
                        InputStream stream = getClass().getResourceAsStream(imagePath);
                        if (stream != null) {
                            profileImg.setImage(new Image(stream));
                        } else {
