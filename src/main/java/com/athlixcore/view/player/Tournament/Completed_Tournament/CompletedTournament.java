...
//         grid.add(createCompletedCard("/assests/images/CompletedTournament.png", "10 - 18 May 2026", "Mumbai Premier League", "Shivaji Park, Mumbai", "Mumbai Masters", "#3b82f6"), 0, 0);
...
public class CompletedTournament {
    ...
    public Node getView() {
        ...
        // Using your new local image for the first card
        grid.add(createCompletedCard("/assests/images/CompletedTournament.png", "10 - 18 May 2026", "Mumbai Premier League", "Shivaji Park, Mumbai", "Mumbai Masters", "#3b82f6"), 0, 0);
        grid.add(createCompletedCard("/assests/images/CompletedTournament1.png", "01 - 08 May 2026", "Pune Super Cricket Cup", "MCA Stadium, Pune", "Pune Strikers", "#10b981"), 1, 0);
        grid.add(createCompletedCard("/assests/images/CompletedTournament2.png", "20 - 28 Apr 2026", "Maharashtra State Championship", "NCA Grounds, Pune", "Royal Titans", "#8b5cf6"), 0, 1);
        grid.add(createCompletedCard("/assests/images/CompletedTournament3.png", "12 - 18 Apr 2026", "Nashik Open Trophy", "Local Grounds, Nashik", "Nashik Warriors", "#f59e0b"), 1, 1);
    ...
    private VBox createCompletedCard(String imageSource, String dateRange, String title, String location, String winnerTeam, String accentColor) {
        // Set padding to 0 on the outer card so the image can stretch edge-to-edge
        ...
        // Robust Image Loader for both URLs and local file paths
        if (imageSource != null && !imageSource.isEmpty()) {
            try {
                if (imageSource.startsWith("http")) {
                    imageView.setImage(new Image(imageSource, true));
                } else {
                    String cleanPath = imageSource.startsWith("/") ? imageSource.substring(1) : imageSource;
                    File file = new File("src/main/resources/" + cleanPath);
                    if (file.exists()) {
                        imageView.setImage(new Image(file.toURI().toString(), true));
                    } else {
                        InputStream stream = getClass().getResourceAsStream(imageSource.startsWith("/") ? imageSource : "/" + imageSource);
                        if (stream != null) {
                            imageView.setImage(new Image(stream));
                        } else {
                            System.out.println("❌ IMAGE NOT FOUND! Expected path: " + file.getAbsolutePath());
                        }
                    }
                }
            } catch (Exception e) {
                System.out.println("❌ Exception loading photo: " + imageSource);
            }
        }
        
        // Bind width to span edge-to-edge (subtracting borders)
        imageView.fitWidthProperty().bind(card.widthProperty().subtract(2)); 
        imageView.setFitHeight(160);
        imageView.setPreserveRatio(false);

        // Clip only the top corners to match the card radius
        ...
        // Container for text and buttons to apply internal padding below the image
        ...
        // Add elements to content block
        contentBox.getChildren().addAll(topInfo, titleLbl, locLbl, winnerBox, viewTournamentBtn);
        
        // Add image and content block to the main card
